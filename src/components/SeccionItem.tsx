...
interface SeccionItemProps {
  ...
  enConflicto: boolean
  conflictosCon?: string[]
  onToggle: () => void
}

export default function SeccionItem({ seccion, seleccionada, enConflicto, conflictosCon = [], onToggle }: SeccionItemProps) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {(enConflicto && seleccionada) || (!seleccionada && conflictosCon.length > 0) ? (
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
        ...
        <button
          onClick={e => { e.stopPropagation(); setDetalleOpen(true) }}
          className="self-center flex-shrink-0 text-[10px] font-medium px-2 py-1 rounded-lg border border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-700 dark:hover:text-slate-200 transition-colors"
        >
          Ver detalles
        </button>
      </motion.div>
      ...
      <SeccionDetailDialog
        ...
        enConflicto={enConflicto && seleccionada}
      />
    </>
  )
}
