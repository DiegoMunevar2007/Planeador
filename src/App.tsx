...
import { SunIcon, MoonIcon, ChevronLeftIcon, ChevronRightIcon, ArrowDownTrayIcon, AcademicCapIcon, XMarkIcon, TrashIcon, VariableIcon, LightBulbIcon, MagnifyingGlassIcon, CheckCircleIcon } from '@heroicons/react/24/solid'
import { useCourses } from './hooks/useCourses'
import { usePlans } from './hooks/usePlans'
import { detectarConflictos, tienenConflicto } from './utils/conflictDetection'
...
export default function App() {
  ...
  const { resultados, loading, loadingMore, hasMore, error, buscar, cargarMas } = useCourses()
  const {
    planes,
    planActivoId,
    seccionesActivas,
  ...
  const [vista, setVista] = useState<'planner' | 'combiner'>('planner')
  ...
  const conflictosActivosMap = useMemo(() => {
    const map = new Map<string, string[]>()
    for (const c of conflictos) {
      const aCodes = map.get(c.seccionA.idUnico) ?? []
      if (!aCodes.includes(c.seccionB.curso.codigo)) aCodes.push(c.seccionB.curso.codigo)
      map.set(c.seccionA.idUnico, aCodes)

      const bCodes = map.get(c.seccionB.idUnico) ?? []
      if (!bCodes.includes(c.seccionA.curso.codigo)) bCodes.push(c.seccionA.curso.codigo)
      map.set(c.seccionB.idUnico, bCodes)
    }
    return map
  }, [conflictos])

  const conflictosCandidatos = useMemo(() => {
    const map = new Map<string, string[]>()
    for (const curso of resultados) {
      for (const sec of curso.secciones) {
        const codes: string[] = []
        for (const activa of seccionesActivas) {
          if (tienenConflicto(sec, activa)) {
            codes.push(activa.curso.codigo)
          }
        }
        if (codes.length > 0) map.set(sec.idUnico, codes)
      }
    }
    return map
  }, [resultados, seccionesActivas])
    ...
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-slate-100">
      ...
      <header className="bg-gradient-to-r from-brand-800 to-indigo-700 dark:from-slate-900 dark:to-slate-800 text-white px-4 md:px-6 py-3 flex items-center justify-between flex-shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button
            ...
            onClick={() => setVista('combiner')}
            title="Combinador automático"
          >
      ...
      {vista === 'combiner' ? (
        <CombinerPage
          ...
          onToggleSeccion={(idUnico) => toggleSeccion(idUnico)}
          onVolver={() => setVista('planner')}
        />
      ) : (
        <>
          {/* Plan tabs */}
            ...
            <AnimatePresence initial={false}>
              {sidebarOpen && (
                <motion.div
                  ...
                  animate={{ width: 420, opacity: 1, overflow: 'hidden' }}
                ...
                >
                  <div className="flex flex-col h-full min-w-0">
                    ...
                    <div className="flex border-b border-gray-200 dark:border-slate-700 flex-shrink-0">
                      ...
                      >
                        <MagnifyingGlassIcon className="size-3 inline-block -mt-0.5 mr-1" />
                        Buscar
                        {pestaña === 'buscar' && (
                          <motion.div
                      ...
                      >
                        <CheckCircleIcon className="size-3 inline-block -mt-0.5 mr-1" />
                        Seleccionadas
                        {seccionesActivas.length > 0 && (
                      ...
                      <button
                        ...
                        onClick={() => setPestaña('sugerencias')}
                      >
                        <LightBulbIcon className="size-3 inline-block -mt-0.5 mr-1" />
                        CBUH/CBUT
                        {pestaña === 'sugerencias' && (
                          <motion.div
                            layoutId="panel-indicator"
                      ...
                      <AnimatePresence mode="wait">
                        {pestaña === 'buscar' ? (
                                ...
                                >
                                  ...
                                  {curso.secciones.map((seccion: Seccion) => (
                                    <SeccionItem
                                      ...
                                      conflictosCon={conflictosCandidatos.get(seccion.idUnico) ?? []}
                              ...
                              {hasMore && (
                                <div className="flex justify-center pt-3 pb-4">
                                  <motion.button
                                    ...
                                    className="flex items-center gap-2 px-5 py-2 text-xs font-semibold rounded-xl bg-brand-800 hover:bg-brand-700 dark:bg-brand-700 dark:hover:bg-brand-600 text-white shadow-sm transition-colors disabled:opacity-60"
                                  >
                                    {loadingMore ? (
                                      <>
                              ...
                              <>
                                ...
                                <AnimatePresence>
                                  {seccionesActivas.map((sec, idx) => {
                                    ...
                                    const conflictCodes = conflictosActivosMap.get(sec.idUnico)
                                        ...
                                        >
                                          <CardContent className="flex gap-2.5 pt-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                              ...
                                              {conflictCodes && conflictCodes.length > 0 && (
                                                <div className="text-[10px] text-red-600 dark:text-red-400 font-medium mb-0.5">
                                                  ⚠ Conflicto con: {conflictCodes.join(', ')}
                                                </div>
                                              )}
                          ...
                          >
                            <SugerenciasTab
                              seccionesActivas={seccionesActivas}
                              onToggleSeccion={(idUnico: string) => toggleSeccion(idUnico)}
                            />
                          </motion.div>
                        )}
                      </AnimatePresence>
